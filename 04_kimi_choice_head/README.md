...
该目录用于复现 Kimi（moonshot-v1-32k）到 Qwen2.5-7B 的两阶段蒸馏流程。它的价值不在于性能提升，而在于保留一个“弱教师且错误分歧未形成有效知识”的负例实验。

## 最新复核结果（2026-05 rerun）

- 教师测试准确率：62.65%（52/83）
- 教师训练集覆盖率：100%（672/672）
- 教师-GT 不一致率：27.83%（187/672）
- 学生最终测试准确率：77.11%（seed=42，与基线持平）
- 结论：Kimi 仍属于弱教师；虽然标签与 GT 存在明显分歧，但这些分歧没有转化为可复现的蒸馏收益，最终结果仍停留在基线水平
