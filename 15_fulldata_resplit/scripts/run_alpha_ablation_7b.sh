...
#
# 与 14B 消融 (run_alpha_ablation.sh) 完全平行, 唯一区别:
...
echo "==============================================="
echo " α-Ablation (7B, Stage-1 only)"
echo " alphas : ${ALPHAS[*]}"
echo " seeds  : ${SEEDS[*]}"
...
if [[ ! -f "$REPORT" ]]; then
  {
    echo "# α-Ablation Rolling Report (7B, Stage-1 only)"
    echo ""
...
echo " Next: canonical eval —"
echo "   python3 $ROOT_DIR/scripts/run_eval_dual.py --run_root $RUN_ROOT --student_size 7b"
echo "==============================================="
