...
Usage:
    python scripts/run_eval_dual.py --run_root runs/XXXX --student_size 7b|14b
"""
import argparse
import json
import os
...
def find_adapters(run_root):
    """Find all adapter directories (stage1_head/best and stage2_sft/best)."""
    adapters = []
...
def main():
    ...
    print(f"\n{'='*60}")
    print(f"Dual Evaluation: {args.student_size.upper()} student")
            ...
            try:
                ...
                for line in output.split("\n"):
                    if "accuracy" in line.lower() or "正确率" in line.lower() or "准确率" in line.lower() or "correct" in line.lower():
                        import re
                        # Try percentage with % sign first
                        m = re.search(r'(\d+\.?\d*)\s*%', line)
                        if m:
                            acc = float(m.group(1))
                        else:
                            # Try fraction like 855/991
                            m = re.search(r'(\d+)\s*/\s*(\d+)', line)
                            if m:
                                acc = 100.0 * int(m.group(1)) / int(m.group(2))
                            else:
                                # Try bare decimal after colon (e.g. "准确率: 86.28")
