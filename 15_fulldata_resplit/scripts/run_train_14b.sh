#!/usr/bin/env bash
set -euo pipefail
# Train 14B student with DeepSeek teacher (Stage 1 ONLY — stage2 SFT is harmful for 14B)

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
source "$ROOT_DIR/../shared/common_env.sh"
resolve_python
resolve_model_dir BASE_MODEL_14B Qwen2.5-14B-Instruct
RUN_ROOT="$ROOT_DIR/runs/$(date +%Y%m%d_%H%M%S)_fulldata_14b"
...
# Parse each config and run stage1 only
"$PY" -c "
