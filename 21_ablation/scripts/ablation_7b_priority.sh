...
set -uo pipefail
# ============================================================
# 7B α-消融 插队编排 (Module 21) — 仿 canonical_eval_alpha0.sh 暂停/恢复范式
#
# 流程:
...
#   4. 7B canonical eval (已验证的 evaluate_model.py 直评, full991+dental125)
#   5. 汇总均值 -> 写日志
...
ABL=21_ablation/runs/alpha_ablation_7b       # 7B 消融产物
LOG=21_ablation/logs/ablation_7b_priority.log
...
log "=== 7B消融插队(Module21): 等当前 Yi34B seed8 完成 ==="
...
bash 21_ablation/scripts/run_alpha_ablation_7b.sh >> 21_ablation/logs/ablation_7b_train_stdout.log 2>&1 \
  && log "7B α-消融 21 点训练完成" || log "WARN 7B α-消融训练非零退出 (检查 logs/ablation_7b_train_stdout.log)"
  ...
  for seed in "${SEEDS[@]}"; do
    ...
    for tname in full dental; do
      ...
      "$PY" "$EVAL" --base_model "$BASE" --adapter_dir "$adir" --test_data "$tfile" \
        ...
        >> "21_ablation/logs/canon7b_a${atag}_s${seed}_${tname}.log" 2>&1 \
...
print("\n=== 7B α-消融 canonical eval 汇总 (Module 21) ===")
