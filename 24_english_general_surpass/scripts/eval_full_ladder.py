#!/usr/bin/env python3
"""英文全科零样本阶梯：测 4 个可部署模型在英文全科(8293题)的零样本准确率。

弱教师 = Qwen3-32B（其零样本 73.84% 即弱教师锚）；强教师 flash = 79.80%（已知）。
"""
import json
import os
import sys
...
OUT = os.path.join(RUN, "full_ladder_zeroshot.json")
TEST_SETS = ["test_medqa", "test_medmcqa", "test_mmlu"]
...
def main():
    ...
    rows = collect_all()
    print(f"英文全科题数: {len(rows)}", flush=True)
    results = {"flash_teacher": FLASH_TEACHER, "n_total": len(rows), "zeroshot": {}}
