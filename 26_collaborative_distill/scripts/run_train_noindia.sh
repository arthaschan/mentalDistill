#!/usr/bin/env bash
# "无印度"重训：Qwen2.5-32B 学生，α=0 纯 GT，1 epoch，训练集去掉 MedMCQA(印度)。
# 之后在无印度测试集(medqa+mmlu)上对比弱教师 Qwen3-32B。
set -uo pipefail
cd "$(dirname "${BASH_SOURCE[0]}")/.."   # -> 26_collaborative_distill/
...
mkdir -p "$out"
echo "[$(date +%H:%M:%S)] TRAIN $name (无印度 10488 题, seed=42)"
"$PY" ../shared/train_choice_head_distill.py \
  --model_name "$STUDENT" \
  --data_path data/train_no_india.jsonl \
