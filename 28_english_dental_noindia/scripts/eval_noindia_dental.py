#!/usr/bin/env python3
"""28 英文牙科·无印度评估：学生(复用27的adapter) vs 弱教师(Qwen3-32B 零样本)。

口径：data/test_no_india_dental.jsonl（501 题 = 官方 181 + 训练集抽出 320，无印度牙科子集）。
注意：501 题仍属小样本，±~2.5pp 噪声，结论以 27（全科 4110）为准。
"""
import json
import os
import sys
...
ADAPTER = "../27_english_general_noindia/runs/32B_noindia_a00_s42"
...
def main():
    print("[学生] 加载 Qwen2.5-32B + adapter(27) ...", flush=True)
    ...
    print("=== 英文牙科·无印度(501题)：学生 vs 弱教师 ===", flush=True)
    d = round(s - t, 2)
