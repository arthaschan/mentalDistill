#!/usr/bin/env python3
"""28 英文牙科·无印度评估（第二名学生 Llama-3.3-70B）：复用27的Llama adapter vs 弱教师(Qwen3-32B 零样本)。

口径：data/test_no_india_dental.jsonl（501 题 = 官方 181 + 训练集抽出 320，无印度牙科子集）。
Llama-70B 需 4bit 加载（bf16 装不下 95GB）。
"""
import json
import os
import sys
...
ADAPTER = "../27_english_general_noindia/runs/Llama70B_noindia_a00_s42"
TEST = "data/test_no_india_dental.jsonl"
...
def main():
    # 学生（Llama-70B + adapter(27)，4bit）
    print("[学生] 加载 Llama-3.3-70B(4bit) + adapter(27) ...", flush=True)
    tok = AutoTokenizer.from_pretrained(STU, trust_remote_code=True)
    if tok.pad_token is None:
        tok.pad_token = tok.eos_token
    base = load_base_model(STU, "4bit", device)   # 4bit 已 device_map，不再 .to()
    ...
    print("\n=== 英文牙科·无印度(501题)：学生(Llama-70B) vs 弱教师 ===", flush=True)
    d = round(s - t, 2)
    print(f"  学生 {s}% vs 老师 {t}%  Δ{d:+}  {'超' if s > t else '不超'}  (501题,±~2.5pp噪声)", flush=True)
