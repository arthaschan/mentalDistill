#!/usr/bin/env python3
"""评估 Qwen3-32B 学生（α=0 × 3 seed）在 4 测试集上的准确率，与 DeepSeek 教师对比。

- bf16 加载；DISTILL_USE_CHAT_TEMPLATE=1 → apply_prompt_template 走 Qwen3 自带模板
  （含 enable_thinking=False 修复，与训练一致）。
- 零样本复用 eval_results_qwen3_zeroshot.json（已用同模板测，73.84%）。
- 产出 runs/eval_results_qwen3.json。
"""
import json
import os
import statistics
...
def main():
    ...
    for seed in SEEDS:
        ad = f"{RUN}/Qwen3_a00_{seed}/best"
        print(f"=== 评估 Qwen3_a00_{seed} ===", flush=True)
    ...
    print("\n=== Qwen3-32B 学生 α=0 结果 (mean±std over 3 seed) ===")
    for t in TEST_SETS:
        m, sd = summary[t]
        t_ = teacher.get(t)
        z = zeroshot[t]
    ...
    json.dump({"teacher": teacher, "zeroshot": zeroshot, "per_seed": per_seed,
              ...
              open(f"{RUN}/eval_results_qwen3.json", "w"), ensure_ascii=False, indent=2)
    print(f"\n-> {RUN}/eval_results_qwen3.json")
