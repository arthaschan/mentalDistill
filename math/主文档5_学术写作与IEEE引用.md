...
# 第八部分：IEEE 真实范例量化（基于已通过评审的 AIEA 论文，可直接照搬）

> 来源：经论文委员会评审+老师审核的标准 IEEE 论文 aiea_DentalMCQ_Distillation。把抽象规则量化成可对照范例。

## 8.1 作者块（多机构）
每机构块四行：作者名 / 系(Dept./School/Faculty) / 机构 / 地区+国家。同机构多作者共用一块，邮箱可 `{u1,u2}@domain` 合并；ORCID 可与邮箱同行；通讯作者标 `*`。

## 8.2 摘要+关键词
- `Abstract—`(破折号)起头，单段~250词，严格走5部件。**数字必须具体**：评审通过的摘要写 `89.10% > 87.18% 教师, 3-seed 88.67%`，不写 "significant improvement"。
- `Keywords—`，分号分隔，~5个。

## 8.3 章节结构（IEEE会议论文实际节序）
Introduction → Method(含子节) → Experimental Setup → Results and Discussion → Practical Implications and Limitations → Conclusion → Acknowledgment → References。
- 贡献点在 Intro 末尾 "The main contributions of this work are as follows:" 列出。
- Results 用**加粗 run-in 小标题**组织：`The Role of Stage 2 SFT Calibration:` 等。

## 8.4 参考文献（按源类型）
- 期刊：`[8] J. Gou, ..., "Knowledge distillation: A survey," Int. J. Comput. Vis., vol. 129, no. 6, pp. 1789-1819, 2021.`
- 会议：`[1] G. Hinton, ..., "Distilling the knowledge in a neural network," in Proc. NIPS ... Workshop, Montreal, Canada, 2015, arXiv:1503.02531.`
- arXiv：`[5] DeepSeek-AI, "DeepSeek-V3 technical report," arXiv preprint arXiv:2412.19437, 2024.`
- 要点：作者 `F. Surname` 逗号分隔末个前加and；**6作者以上 et al.**；标题双引号句式大小写；期刊/会议名标准缩写+斜体；卷期页 `vol.x, no.x, pp.xx-xx`；月份缩写 `Jun. 2024`；新式期刊 `Art. no. 109296`；机构作者直接写名；按正文出现顺序编号。

## 8.5 图/表/公式
- 图题注在**图下方**，`Fig. N.`(缩写+句点)，是**完整句常含关键数字**。正文 `Fig. 2 visualizes these gains.`
- 表题注在**表上方**，`TABLE I.`(全大写罗马数字)，可加缩写定义。
- 公式右侧编号 (1)(2)(3)；正文 `Equations (1), (2), and (3)`，不写 "equation (2) of [5]"。

## 8.6 数字/百分比
两位小数+%：`89.10%`；百分点差 `a gain of 9.11 percentage points`；区间 en-dash `88.40%–89.10%`；样本数带逗号 `4,608`；学习率 `1 × 10⁻⁴`；超参直接给值 `rank 16 and LoRA alpha 32`。

## 8.7 局限段真实写法（印证诚实护栏）
评审通过的 Limitations 照搬语气：明确声明缺对照("does not include a direct full-vocabulary distillation control")、不过度外推("evaluates a single strong teacher ... not enough to show that every teacher-student pair will behave the same")、点明边界("should not be broadly generalized to open-ended clinical dialogue")。

## 8.8 致谢
`Acknowledgment`(IEEE单数拼写)：`The work ... was supported by ...`

---
> 本主文档自包含 = 写作质量(A-E)+判断框架+摘要模型+结构模型+对冲语保护+IEEE引用+中英双语工作流+真实范例。
> 待办：可提炼为 Hermes skill `academic-writing-guidance`(触发：写论文/IEEE引用/中英双语时)。
