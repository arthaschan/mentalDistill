...
> 导师姓名（职称）：熊体超博士（副教授）日期： 2026-05

![A logo with text and a wave AI-generated content may be incorrect.](thesis/docx_media/media/image1.jpeg){width="1.6625in" height="1.3680555555555556in"}
...
Applied Artificial Intelligence of

HONG KONG CHU HAI COLLEGE

> Supervisor: Dr. Richard Tai-Chiu Hsung Date: MAY 2026
...
本文通过系统性的实验开展研究，包括蒸馏等方法研究中观察到一些值得注意的发现。（1）当前的大语言模型在进行压缩的同时，仍能达到并优于现有教师模型的性能

（2）目前的最先进模型（如Qwen、Deepseek 等）的蒸馏结果显示，教师模型质量与蒸馏之间存在"倒 U 型"关系

3.  基于 Fisher-Rao 距离与 α-散度的分析表明，真实 logprobs 标签较人工平滑标签保留了更丰富的连续结构信息，其体积密度高约 2500 倍；

4.  异构教师同样可以提供有效蒸馏信号，72.45% 准确率的 Llama-3.3-70B 教师仍能蒸馏出达到 87.25% 的 14B 学生模型。
...
> [1.4 研究贡献 11](\l)
>
> [1.5 研究计划与技术路线调整 12](\l)
>
> [1.6 论文结构 12](\l)

[第二章 研究背景与理论基础 12](\l)

> [2.1 医疗人工智能聊天机器人研究现状 12](\l)
>
> [2.2 知识蒸馏技术研究进展 13](\l)
>
> [2.3 医疗大语言模型发展现状 15](\l)
>
> [2.4 信息几何理论基础 16](\l)
>
> [2.5 技术路线总览 17](\l)
...
> [3.2 数据集构建 18](\l)
>
> [3.2.1 CMExam 牙科子集（小数据集） 18](\l)
>
> [3.2.2 全量数据重分割（大数据集） 18](\l)
>
> [3.2.3 教师标签生成 18](\l)
>
> [3.3 Choice-Head 两阶段蒸馏 19](\l)
>
> [3.3.1 Stage 1：Choice-Head KL 蒸馏 20](\l)
>
> [3.3.2 Stage 2：GT SFT 精校 20](\l)
>
> [3.3.3 两阶段策略分析 21](\l)
>
> [3.4 白盒 Logit-KL 蒸馏 21](\l)
>
> [3.5 多教师融合策略 22](\l)
>
> [3.5.1 静态标签融合（Module 07） 22](\l)
>
> [3.5.2 多票自一致性过滤（Module 08） 22](\l)
>
> [3.6 信息几何引导优化 22](\l)
>
> [3.6.1 α-散度替代 KL（Module 17） 22](\l)
>
> [3.6.2 Fisher-Rao 距离教师质量分析（Module 18） 23](\l)
>
> [3.6.3 边界距离置信度过滤（Module 19） 23](\l)
>
> [3.6.4 样本自适应 α-散度（Module 20） 23](\l)
>
> [3.7 LoRA 微调配置 23](\l)
>
> [3.8 系统部署架构 23](\l)
...
> [4.2 基线与教师能力上界 25](\l)
>
> [4.2.1 学生基线（Module 00） 25](\l)
>
> [4.2.2 教师能力谱系 25](\l)
>
> [4.3 白盒蒸馏（Module 01） 26](\l)
>
> [4.4 单教师黑盒 Choice-Head（Modules 02-06） 26](\l)
>
> [4.4.1 总体对比 26](\l)
>
> [4.4.2 DeepSeek-V3 详细结果（Module 02） 28](\l)
>
> [4.4.3 豆包详细结果（Module 03） 28](\l)
>
> [4.4.4 Kimi 复核结果（Module 04） 28](\l)
>
> [4.5 多教师融合与一致性过滤（Modules 07-08） 28](\l)
>
> [4.5.1 静态融合（Module 07） 28](\l)
>
> [4.5.2 多票一致性过滤（Module 08） 28](\l)
>
> [4.6 集成与 CoT 蒸馏（Modules 11-12） 28](\l)
>
> [4.6.1 多 seed 集成（Module 11） 28](\l)
>
> [4.6.2 CoT 蒸馏结果分析（Module 12） 28](\l)
>
> [4.7 学生容量升级（Modules 13-14） 29](\l)
>
> [4.7.1 Qwen2.5-14B（Module 13） 29](\l)
>
> [4.7.2 Qwen3-14B（Module 14） 29](\l)
>
> [4.8 全量重分割验证（Module 15） 29](\l)
>
> [4.8.1 991题零样本基线 29](\l)
>
> [4.8.2 蒸馏结果 30](\l)
>
> [4.8.3 核心发现 31](\l)
>
> [4.9 跨架构蒸馏（Module 16） 32](\l)
>
> [4.10 信息几何实验（Modules 17-20） 32](\l)
>
> [4.10.1 α-散度（Module 17） 32](\l)
>
> [4.10.2 Fisher-Rao 分析（Module 18） 33](\l)
>
> [4.10.3 边界距离过滤（Module 19） 33](\l)
>
> [4.10.4 自适应 α（Module 20） 33](\l)
>
> [4.11 错误分析 34](\l)
>
> [4.12 全局结果汇总 34](\l)
...
围绕这一总体目标，本文设置了四项具体任务：（1）基于 CMExam 数据集建立面向牙科选择题任务的数据划分与评测设置；（2）设计并验证 Choice-Head 蒸馏框架，使其同时兼容黑盒 API 教师与白盒本地教师；（3）分析学生模型在不同教师、不同训练阶段和不同数据规模下的性能变化，并验证学生超越教师的条件；（4）实现可交互的牙科问答原型系统，为后续教学演示与实际部署提供接口基础。
...
但当蒸馏对象从图像分类网络转向大语言模型时，经典方案会立刻遇到任务与系统层面的双重约束。第一，词表规模膨胀使全 vocab logits 的存储与计算代价迅速上升，例如 Qwen2.5 的输出空间已达到 151,936 维\[4\]。第二，许多高性能教师仅以远程云端 API 服务形式提供，外部研究者通常只能访问输入输出接口，而无法稳定获得 logits、隐藏状态等完整内部信息，传统白盒蒸馏因此失去适用前提。第三，大语言模型通常服务于具体任务场景，不同任务对蒸馏信号的需求差异很大，通用式蒸馏目标未必最有效\[5,19,20\]。

在这种背景下，任务结构本身就成为重新定义蒸馏目标的依据。对于五选一医学选择题，教师真正需要传递给学生的信息不在于完整词表上的概率细节，而在于 A/B/C/D/E 五个候选项之间的相对支持关系。沿着任务蒸馏与面向下游决策空间压缩监督目标的思路\[19,20\]，本文将原本针对通用 token 分布的蒸馏过程改写为面向五维选择头的决策分布迁移，并将这一任务化输出头定义为 Choice-Head。这样处理后，无论教师来自本地模型还是远程 API 服务，只要能够提供选项级分布或可恢复的选项信息，就有可能形成有效监督。
...
当前蒸馏研究仍面临一个关键但常被弱化的问题：教师信息究竟应以何种形式被学生继承，才能最大程度保留对最终任务有用的决策结构\[5,19,20\]。在传统分类任务中，类别数固定且较少，软标签能够直接表达类别相似性，因此 logit-level distillation 往往是自然选择\[5,20\]。但在大语言模型任务中，完整词表分布既极高维，又包含大量与当前题目无关的 token，直接匹配全词表输出会同时带来计算负担和任务噪声\[4,5,19\]。现有方法大体可以分为全词表分布匹配、隐层或特征对齐以及面向下游任务输出的蒸馏三类，它们分别适合白盒教师、可访问中间层的训练框架以及结构明确的任务场景\[5,19,20\]。Choice-Head 的出发点正是在这一比较基础上形成的：对于五选一医学选择题，真正需要迁移的不是整个语言生成空间，而是教师如何在有限候选项之间分配支持度；因此本文选择把监督对象收缩到五维选项头，而不是继续沿用通用 token 级蒸馏\[5,19,20\]。
...
在 $n$ 类分类任务中，模型的输出概率分布 $p = (p_{1},p_{2},\ldots,p_{n})$ 构成一个 $(n - 1)$ 维概率单纯形。Fisher 信息度量 $g_{\text{ij}}(p) = \delta_{\text{ij}}/p_{i}$ 赋予该单纯形一个黎曼结构\[18\]。Fisher-Rao 距离定义为：

$$d_{\text{FR}}(p,q) = 2\text{arccos}\left( \sum_{i}^{}\sqrt{p_{i}q_{i}} \right)$$

Amari 的 $\alpha$-散度是 KL 散度的自然推广\[17\]：

$$D_{\alpha}(p \parallel q) = \frac{4}{1 - \alpha^{2}}\left( 1 - \sum_{i}^{}p_{i}^{(1 + \alpha)/2}q_{i}^{(1 - \alpha)/2} \right)$$

当 $\alpha \rightarrow 1$ 时退化为前向 KL 散度（mode-seeking），$\alpha = 0$ 时正比于 Hellinger 距离的平方（对称、有界），$\alpha \rightarrow - 1$ 时退化为逆向 KL 散度（mode-covering）。不同 $\alpha$ 值对应不同的几何特性，在蒸馏中产生不同的梯度行为。
...
![](thesis/docx_media/media/image2.png){width="6.141666666666667in" height="3.509027777777778in"}
...
为便于理解数据格式，下面给出一条牙科子集样例：

题干：上皮基层下疱常见于

选项：A 红斑；B 红斑狼疮；C 天疱疮；D 类天疱疮；E 白斑

标准答案：D

说明：该样例展示了本文使用的五选一 JSONL 数据结构，其核心字段包括题干、五个候选项与标准答案，能够直接映射到后续 Choice-Head 蒸馏所需的选项级监督形式。

具体内容如下：

{\"Question\": \"上皮基层下疱常见于\", \"Options\": \"A 红斑\\nB 红斑狼疮\\nC 天疱疮\\nD 类天疱疮\\nE 白斑\", \"Answer\": \"D\", \"Explanation\": \"基层下疱：疱在基底层之下，基底细胞变性，使上皮全层与固有层分离。见于黏膜良性类天疱疮、多形渗出性红斑。掌握"口腔黏膜基本病理变化"知识点。\", \"Disease Group\": \"消化系统疾病\", \"Area of Competency\": \"疾病诊断和鉴别诊断\", \"Clinical Department\": \"口腔科\", \"Medical Discipline\": \"口腔医学\", \"Difficulty level\": 1, \"source\": \"cmexam\"}
...
![](thesis/docx_media/media/image3.png){width="6.2125in" height="2.74375in"}

图 3-1 Choice-Head 两阶段蒸馏流程图。
...
具体实现时，对于学生模型的答案位置，提取 A/B/C/D/E 对应 token 的 logit，并经 softmax 归一化得到选项概率分布 $p_{S} \in \mathbb{R}^{5}$：

$$p_{S} = \text{softmax}(z_{S}\lbrack t_{A},t_{B},t_{C},t_{D},t_{E}\rbrack)$$

其中 $t_{A},t_{B},t_{C},t_{D},t_{E}$ 分别表示五个选项字母对应的 token ID。若教师来自本地白盒模型，可直接读取其对应的选项分布；若教师来自 API，只需获得选项级概率或可恢复的 logprobs 信息即可。

Stage 1 的训练目标为：

$$L_{\text{stage1}} = \alpha \cdot D_{\text{KL}}(p_{T} \parallel p_{S}) + (1 - \alpha) \cdot L_{\text{CE}}$$

其中 $p_{T}$ 为教师软标签，$\alpha = 0.35$ 为默认蒸馏权重。该阶段通常只训练 1 个 epoch，重点不是让学生立即完全收敛，而是先学习教师对干扰项与正确项之间的相对判断模式。
...
Stage 2 通常训练 2--5 个 epoch。这里所说的教师"暗知识"，主要是指教师在 A/B/C/D/E 五个选项上的概率结构信息，包括正确项与次优项之间的置信差距，以及各干扰项之间的相对排序。Stage 2 的目标是在尽可能保留这类选项级结构信息的基础上，借助 GT 标签将学生模型的决策结果进一步校准到标准答案，从而兼顾知识迁移效果与答案对齐能力。

Stage 2 继承 Stage 1 训练得到的 LoRA 权重，使用标准答案进行纯监督微调（SFT），目的是纠正 Stage 1 中因教师错误预测导致的偏移：

$$L_{\text{stage2}} = - \text{log}P_{\theta}(y^{\ast} \mid x)$$
...
$$L_{\text{whitebox}} = \alpha T^{2}D_{\text{KL}}(\sigma(z_{T}/T) \parallel \sigma(z_{S}/T)) + (1 - \alpha)L_{\text{CE}}$$
...
$$p_{\text{fused}} = \sum_{k}^{}w_{k}p_{T_{k}},\quad w_{k} = \frac{\text{ac}c_{k}}{\sum_{j}^{}acc_{j}}$$
...
$$D_{\alpha}(p_{T} \parallel p_{S}) = \frac{4}{1 - \alpha^{2}}\left( 1 - \sum_{i}^{}p_{T_{i}}^{(1 + \alpha)/2}p_{S_{i}}^{(1 - \alpha)/2} \right)$$
...
基于教师答错样本的边界距离是答对样本 2 倍的发现，设计了基于边界距离的数据过滤策略。Hard 过滤设阈值 $d_{b}^{\ast}$，仅保留 $d_{b} < d_{b}^{\ast}$ 的样本使用教师标签蒸馏，其余样本回退到 GT 标签。Soft 加权用 sigmoid 函数对蒸馏权重进行连续调整：

$$w_{i} = \sigma\left( - \frac{d_{b} - d_{b}^{\ast}}{0.01} \right)$$

### 3.6.4 样本自适应 α-散度（Module 20）

基于 KL 种子敏感性最大、Hellinger 更稳健的发现，设计了根据样本边界距离动态选择 $\alpha$ 值的策略：

$$\alpha_{i} = \sigma\left( \frac{d_{\text{boundary}} - \tau}{\gamma} \right)(\alpha_{\text{high}} - \alpha_{\text{low}}) + \alpha_{\text{low}}$$
...
![图 4-1 教师质量与蒸馏收益关系图。横轴为教师与 GT 的不一致率，纵轴为学生相对基线增益。](thesis/docx_media/media/image4.png){width="6.208333333333333in" height="3.7125in"}

图 4-1 教师质量与蒸馏收益关系图。横轴为教师与 GT 的不一致率，纵轴为学生相对基线增益。
...
表 4-2 单教师黑盒 Choice-Head 蒸馏总体结果
  ...
  04     Kimi                62.65%       77.11%     77.11%       ±0.00
...
补充说明：Module 04 的 Kimi 行采用 2026-05 复核口径，教师测试准确率为 62.65%（52/83），训练集不一致率为 27.83%（187/672），对应 seed=42 两阶段重训后的测试准确率为 77.11%。

图 4-2 在统一坐标系中并列展示教师准确率、学生峰值与学生稳定值。该图相较表 4-2 更能突出两点：其一，教师性能并不与蒸馏收益形成简单正相关；其二，真实 logprobs 异构教师仍可提供具有竞争力的蒸馏信号。

![图 4-2 单教师 Choice-Head 蒸馏结果对比图。并列展示教师准确率、学生峰值与学生稳定值。](thesis/docx_media/media/image5.png){width="6.208333333333333in" height="3.4555555555555557in"}

图 4-2 单教师 Choice-Head 蒸馏结果对比图。并列展示教师准确率、学生峰值与学生稳定值。
...
![图 4-3 全量测试集与牙科子集零样本基线对比图。深灰柱为 991 题全量测试集，白色点纹柱为 125 题牙科子集。](thesis/docx_media/media/image6.png){width="6.208333333333333in" height="3.4763888888888888in"}

图 4-3 全量测试集与牙科子集零样本基线对比图。深灰柱为 991 题全量测试集，白色点纹柱为 125 题牙科子集。
...
![图 4-4 全量重分割蒸馏结果对比图。斜线灰柱表示全量均值，深灰柱表示全量最佳，白色点纹柱表示牙科均值。](thesis/docx_media/media/image7.png){width="6.208333333333333in" height="3.5243055555555554in"}

图 4-4 全量重分割蒸馏结果对比图。斜线灰柱表示全量均值，深灰柱表示全量最佳，白色点纹柱表示牙科均值。
...
结果表明，smooth_eps 标签的熵与峰值几乎恒定，暗知识有限；真实 logprobs 则在概率流形上提供了更细粒度的混淆结构。具体而言，Llama-70B 真实 logprobs 标签的 $\text{log}_{10}\sqrt{\text{det}g}$ 约为 6.05，而 smooth_eps 标签约为 2.65，对应体积元素相差 $10^{6.05 - 2.65} \approx 10^{3.4} \approx 2500$ 倍。这里统一表述为"约 2500 倍"，以强调其数量级差异，证实了两类标签在几何性质上的本质差异。
...
![图 4-5 代表性实验结果排序图。深色条表示蒸馏后学生结果，浅色条表示教师或零样本基线。](thesis/docx_media/media/image8.png){width="6.208333333333333in" height="3.6743055555555557in"}

图 4-5 代表性实验结果排序图。深色条表示蒸馏后学生结果，浅色条表示教师或零样本基线。
...
我还要感谢patrick,是我们人工智能实验室的运维人员，我使用学校资源h100，得到了他的不少帮助。
...
# 学位论文版权使用授权声明 {#学位论文版权使用授权声明-1}

本学位论文作者完全了解香港珠海学院有关保留、使用学位论文的规定，即：学校有权保留并向有关部门或机构送交论文的复印件和电子版，允许论文被查阅和借阅。

学位论文作者签名：陈天元导师签名：\_\_\_\_\_\_\_\_\_\_\_\_\_\_

日期：2026年5月19日 日期：\_\_\_\_年 \_ 月 \_ 日
