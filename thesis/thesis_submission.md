...
但当蒸馏对象从图像分类网络转向大语言模型时，经典方案会立刻遇到任务与系统层面的双重约束。第一，词表规模膨胀使全 vocab logits 的存储与计算代价迅速上升，例如 Qwen2.5 的输出空间已达到 151,936 维[4]。第二，许多高性能教师以 API 形式存在，外部研究者无法稳定获得完整内部状态，传统白盒蒸馏因此失去适用前提。第三，大语言模型通常服务于具体任务场景，不同任务对蒸馏信号的需求差异很大，通用式蒸馏目标未必最有效。
